...
<body>
				...
				<ul class="children collapse" id="sub-item-1">
					<li >
						<a class="" href="makeUpClass.php">
							<svg class="glyph stroked chevron-right active"><use xlink:href="#stroked-chevron-right"></use></svg> Add Alternative Class
						</a>
					</li>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="panel panel-default">
				<div class="panel-heading">Add Alternative Class</div>
				<div class="panel-body">
		...
		<?php
			if(isset($_POST['create'])){
				...
				if(empty($_POST['room'])){
					...
					$printresult = mysqli_query($dbc,$query);
					$row3 = mysqli_fetch_array($printresult, MYSQL_ASSOC);
					if($row3['ROOM'] != NULL){
					...
					if (substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),0,1) == 'T') {
						if (strtoupper(substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),1,1)) == 'U') {
							...
							if (substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),0,1) == "{$row4['dayid']}" &&
								...
								$altendtime >= "{$row4['starttime']}" || $altendtime <= "{$row4['endtime']}") {
							...
							if (strtoupper(substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),1,1)) == "{$row4['dayid']}" &&
								$altstarttime >= "{$row4['starttime']}" || $altstarttime <= "{$row4['endtime']}" ||
								$altendtime >= "{$row4['startime']}" || $altendtime <= "{$row4['endtime']}"){
						...
						if (substr(date('D', mktime(0,0,0, $mmonth, $mday, $myear)),0,1) == "{$row4['dayid']}" &&
							$altstarttime >= "{$row4['starttime']}" || $altstarttime <= "{$row4['endtime']}" ||
							$altendtime >= "{$row4['starttime']}" || $altendtime <= "{$row4['endtime']}"){
				...
				$makeupval = 0;
				$makeup = "SELECT makeupdate, makeupstarttime, makeupendtime from mv_facultymakeup";
				...
				while ($row5=mysqli_fetch_array($getmakeup, MYSQLI_ASSOC)){
					$madate = new DateTime($row5['makeupdate']);
					$maday = date_format($madate, 'd');
					$mamonth = date_format($madate, 'm');
					$mayear = date_format($madate, 'Y');

					$date = substr(date('D', mktime(0,0,0, $mamonth, $maday, $mayear)),0,1);

					if (substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),0,1) == 'T') {
						if (strtoupper(substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),1,1)) == 'U') {

							if (substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),0,1) == "$date" &&
								$altstarttime >= "{$row5['makeupstarttime']}" || $altstarttime <= "{$row5['makeupendtime']}" ||
								$altendtime >= "{$row5['makeupstarttime']}" || $altendtime <= "{$row5['makeupendtime']}") {
							...
							if (strtoupper(substr(date('D', mktime(0,0,0, $amonth, $aday, $ayear)),1,1)) == "$date" &&
								$altstarttime >= "{$row5['makeupstarttime']}" || $altstarttime <= "{$row5['makeupendtime']}" ||
								$altendtime >= "{$row5['makeupstartime']}" || $altendtime <= "{$row5['makeupendtime']}"){
							...
							if (substr(date('D', mktime(0,0,0, $mmonth, $mday, $myear)),0,1) == "$date" &&
								$altstarttime >= "{$row5['makeupstarttime']}" || $altstarttime <= "{$row5['makeupendtime']}" ||
								$altendtime >= "{$row5['makeupstarttime']}" || $altendtime <= "{$row5['makeupendtime']}"){
